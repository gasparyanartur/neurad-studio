...
class LossSettings:
    ...
    """Multiplier for the diffusion loss ."""
...
@dataclass
class NeuRADDiffusionModelConfig(NeuRADModelConfig):
    """Overall configuration of NeuRAD."""

    _target: Type = field(default_factory=lambda: NeuRADDiffusionModel)
...
class NeuRADDiffusionModel(NeuRADModel):
    ...
    config: NeuRADDiffusionModelConfig

    def populate_modules(self):
        """Set the fields and modules."""
        super().populate_modules()
        if self.config.implementation == "tcnn" and not TCNN_EXISTS:
            print_tcnn_speed_warning("NeuRADDiffusion")
